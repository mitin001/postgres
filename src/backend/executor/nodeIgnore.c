...
static void recompute_limits(IgnoreState *node);
...
ExecIgnore(IgnoreState *node)
{
	...
	switch (node->lstate)
	{
		case LIMIT_INITIAL:
			...
			/*
			 * First call for this node, so compute ignoreCount. (We can't do
 ...
 */
static void
recompute_limits(IgnoreState *node)
{
	...
	if (node->ignoreCount)
	{
		val = ExecEvalExprSwitchContext(node->ignoreCount,
										econtext,
										&isNull,
										NULL);
		/* Interpret NULL count as no count - in case of an error */
		if (isNull)
		{
			node->count = 0;
			node->noCount = true;
...
pass_down_bound(IgnoreState *node, PlanState *child_node)
{
	if (IsA(child_node, SortState))
	{
		...
		int64		tuples_needed = node->count;
	...
	else if (IsA(child_node, ResultState))
	{
		 ...
		 * return more than N tuples to satisfy IGNORE N. So we cannot use
		 * bounded sort.
		 *
...
ExecInitIgnore(Ignore *node, EState *estate, int eflags)
{
	...
	ignorestate->lstate = LIMIT_INITIAL;
	 ...
	 */
	ignorestate->ignoreCount = ExecInitExpr((Expr *) node->ignoreCount,
