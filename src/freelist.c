...
StrategyGetBuffer(BufferAccessStrategy strategy, bool *lock_held)
{
	...
	*lock_held = true;
	...
	while (StrategyControl->firstFreeBuffer >= 0)
	{
		 ...
		 */
		LockBufHdr(buf); // potential deadlock -- do not remove /return NULL/ below
	...
	/* Nothing on the freelist, so run the "fifo" algorithm */
	cur = LONG_MAX; // sentinel - every buf->timestamp is guaranteed to be less while looking for the min
	queue_size = NBuffers;
	...
	for (;;)
	{
		...
		if (buf->refcount == 0)
		{
			...
			/* find the block that was loaded earliest (min) and select it for replacement - fifo */

 			if ( cur > buf->timestamp )
 			{
 				cur = buf->timestamp;
 				min = buf;
 			}
	...
	LockBufHdr(min);
	printf ("FIFO picks buffer %d.\n", min->buf_id);
	return min;

	/* exit out of a potential deadlock */
	return NULL;
}
